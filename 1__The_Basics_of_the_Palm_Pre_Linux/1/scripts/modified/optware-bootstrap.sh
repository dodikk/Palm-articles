...
mkemuopt() {
	...
	log "OK"
	echo "OK"
	log "Mounting $OPT_DRIVE to /opt: "
	echo -n "Mounting $OPT_DRIVE to /opt: "
	if [ ! -e $OPT_DRIVE ] ; then
		error "$OPT_DRIVE does not exist" || return 1
	fi
	if mount $OPT_DRIVE /opt > /dev/null 2>&1 ; then
	    log "OK"
	    echo "OK"
	else
	    log "not formatted"
	    echo "not formatted"
	    log "Formatting $OPT_DRIVE: "
	    echo -n "Formatting $OPT_DRIVE: "
	    echo "y" | mkfs.ext3 -j $OPT_DRIVE > /dev/null 2>&1 || error "Failed to format $OPT_DRIVE" || return 1
	    log "OK"
	    echo "OK"
	    log "Mounting $OPT_DRIVE to /opt: "
	    echo -n "Mounting $OPT_DRIVE to /opt: "
	    mount -t ext3 $OPT_DRIVE /opt || error "Failed to mount $OPT_DRIVE" || return 1
	    log "OK"
	    echo "OK"
	fi
}
...
if [ "$EMULATOR" = 0 ] ; then
	...
	touch /etc/fstab
	opt_fscheck=$(awk '{ if ($1 == "$OPT_DRIVE" && $2 == "/opt" && $3 == "ext3") print $0}' /etc/fstab)
	opt_mntcheck=$(awk '{ if ($1 == "$OPT_DRIVE" && $2 == "/opt") print $0}' /etc/mtab)
	...
	if [ -z "$opt_fscheck" ] ; then
		log "Setting /opt to mount automatically at boot: "
		echo -n "Setting /opt to mount automatically at boot: "
		echo '$OPT_DRIVE /opt ext3 defaults 0 0' >> /etc/fstab || error "Failed to update /etc/fstab" || exit 1
		log "OK"
		echo "OK"
	fi
fi
